    ...
    class joinbot
    {
        ...
        public void Bot(string token, int botnum)
        {
                    ...
                    if (!e.Channel.IsPrivate)
                    {
						...
						if (isOwner && cmd == "eval")
						{
							try
							{
								...
								var parameters = new[] {
									new Parameter("e", e),
									new Parameter("_client", _client),
									new Parameter("htc", htc),
									new Parameter("serv", serv),
									new Parameter("user", user),
									new Parameter("bot", bot),
									new Parameter("owner", owner)
								};

								var result = interpreter.Eval(rawargs, parameters).ToString();
							...
							{
								Console.WriteLine($"[Error] Bot ran into an issue while trying to run \"{rawargs}\": {error.ToString()}");
							}
						}
						...
						else if ((isOwner || e.Channel.Id == 282500390891683841) && cmd == "topusers")
						{
						...
						else if (isOwner && (cmd == "restart" || cmd == "shutdown"))
						{
            ...
            _client.UserJoined += async (s, e) =>
            {
                ...
                else if (e.Server.Id == 282219466589208576)
                {
		...
		public void MessageIncrement(int botnum, User user, ulong sid)
		{
            ...
            var path = $"{Directory.GetCurrentDirectory() + sepchar + botstring + sepchar + sid.ToString() + sepchar}"; // get directory of data file
